...
if not args.skip_training:
    ...
    if args.mipnerf360:
        for scene in mipnerf360_outdoor_scenes:
            ...
            os.system(f"python train0921.py -s {source} -i images_4 -m {args.output_path}/{scene}{common_args}")    
        for scene in mipnerf360_indoor_scenes:
            source = os.path.join(args.mipnerf360, scene)
            os.system(f"python train0921.py -s {source} -i images_2 -m {args.output_path}/{scene}{common_args}")

    # 训练 Tanks and Temples
    if args.tanksandtemples:
        for scene in tanks_and_temples_scenes:
            source = os.path.join(args.tanksandtemples, scene)
            os.system(f"python train0921.py -s {source} -m {args.output_path}/{scene}{common_args}")
