...
renderCUDA(
	...
	int* __restrict__ accum_faint_count,
	float* __restrict__ accum_occluded_alpha,
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (alpha < 1.0f / 255.0f){
				atomicAdd(&(accum_faint_count[collected_id[j]]), 1);
				continue;
			}
			...
			if (test_T < 0.0001f)
			{
				...
				atomicAdd(&(accum_occluded_alpha[collected_id[j]]), alpha);
				continue;
			}
...
void FORWARD::render(
	...
	int* accum_faint_count,
	float* accum_occluded_alpha,
