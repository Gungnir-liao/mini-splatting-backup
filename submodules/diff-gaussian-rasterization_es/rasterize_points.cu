  ...
  )
{
  ...
  torch::Tensor accum_faint_count = torch::full({P}, 0, int_opts);
  torch::Tensor accum_occluded_alpha = torch::full({P}, 0, float_opts);
  ...
  if(P != 0)
  {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
      ...
      accum_faint_count.contiguous().data<int>(),  
      accum_occluded_alpha.contiguous().data<float>(),  
    
      radii.contiguous().data<int>(),
      debug);
  }

  return std::make_tuple(rendered, out_color, accum_weights_ptr, accum_weights_count, accum_max_count, accum_faint_count, accum_occluded_alpha, radii, geomBuffer, 
