...
if not args.skip_training:
    ...
    if args.mipnerf360:
        ...
        for scene in mipnerf360_outdoor_scenes:
            ...
            os.system(f"python train.py -s {source} -i images_4 -m {args.output_path}/{scene}{common_args_m360} --imp_metric outdoor")    
        for scene in mipnerf360_indoor_scenes:
            source = os.path.join(args.mipnerf360, scene)
            os.system(f"python train.py -s {source} -i images_2 -m {args.output_path}/{scene}{common_args_m360} --imp_metric indoor")
    ...
    if args.tanksandtemples or args.deepblending:
        ...
        if args.tanksandtemples:
            for scene in tanks_and_temples_scenes:
                ...
                os.system(f"python train.py -s {source} -m {args.output_path}/{scene}{common_args_others} --imp_metric outdoor")
        
        if args.deepblending:
            for scene in deep_blending_scenes:
                source = os.path.join(args.deepblending, scene)
                os.system(f"python train.py -s {source} -m {args.output_path}/{scene}{common_args_others} --imp_metric indoor")
