...
def main_sampling_loop():
    ...
    R_LEVELS = np.linspace(2, 8, 32, endpoint=False)
    print(f'len:{len(R_LEVELS)},R_LEVELS:{R_LEVELS}')
    A_LEVELS = np.linspace(0, 360, 32, endpoint=False)
    print(f'len:{len(A_LEVELS)},A_LEVELS:{A_LEVELS}')
    A_LEVELS_RAD = np.radians(A_LEVELS)
    E_LEVELS = np.linspace(-90, 90, 32, endpoint=False)
    print(f'len:{len(E_LEVELS)},E_LEVELS:{E_LEVELS}')
    E_LEVELS_RAD = np.radians(E_LEVELS)
    ...
    with open("viewports_20260105.json", "w", encoding="utf-8") as f:
        json.dump(all_infos, f, indent=2)

    print("已生成 viewports_20260105.json，共", len(all_infos), "个视点")
