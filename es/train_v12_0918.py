...
PROFILE_PRUNE_START_ITER = 10000
# 画像裁剪持续的迭代数（统计窗口长度）
PRUNE_WINDOW_ITERS = 10000
# 目标：窗口内总计裁掉占比（相对第一次裁剪时的点数 N0）
PRUNE_TARGET_FRAC = 0.9
# 固定分位阈值（候选筛选用，不退火）
PRUNE_Q = 0.12
# ================================================================

def gaussian_size_penalty(area_frac: torch.Tensor,
                          vis_mask: torch.Tensor,
                          thresh_frac: float = 1.0/50.0,   # 面积占比阈值：例子=2%
                          mode: str = "violator_mean",     # "violator_mean" | "topk" | "huber"
                          topk_ratio: float = 0.1,         # 仅用于 "topk"
                          huber_delta: float = 0.01        # 仅用于 "huber"；按需调
                          ) -> torch.Tensor:
    """
    area_frac: [N] 每个高斯的屏幕面积占比（render_es返回）
    vis_mask:  [N] 可见掩码（radii>0）
    """
    # 只在可见的高斯上考虑
    if vis_mask is not None:
        area_frac = area_frac[vis_mask]
    if area_frac.numel() == 0:
        return torch.zeros((), device=area_frac.device, dtype=area_frac.dtype)

    # 超额
    excess = torch.relu(area_frac - float(thresh_frac))  # [M]，M=可见者数量
    if excess.numel() == 0:
        return torch.zeros((), device=area_frac.device, dtype=area_frac.dtype)

    mode = mode.lower()
    if mode == "violator_mean":
        # 只对超额者求均值：避免被大量正常值稀释
        violator = excess > 0
        return excess[violator].mean() if violator.any() else torch.zeros((), device=excess.device, dtype=excess.dtype)

    elif mode == "topk":
        flat = excess.flatten()
        k = max(1, int(math.ceil(topk_ratio * flat.numel())))
        ...
        # 对超额量做 Huber，既稳健又有梯度
        abs_e = excess.abs()
        quad = torch.minimum(abs_e, torch.tensor(huber_delta, device=abs_e.device, dtype=abs_e.dtype))
        lin  = abs_e - quad
...
def size_penalty_power(area_frac, vis_mask, thresh_frac: float, p: float = 2.0):
    x = torch.relu(area_frac - float(thresh_frac))
    if vis_mask is not None:
        x = x[vis_mask]
    if x.numel() == 0 or x.max() == 0:
        return torch.zeros((), device=area_frac.device, dtype=area_frac.dtype)
    # 只对超额者聚合
    viol = x > 0
    return x[viol].pow(p).mean()
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        image, viewspace_point_tensor, visibility_filter, radii, ellipse_area_px, ellipse_area_frac = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
            render_pkg["visibility_filter"],
            render_pkg["radii"],
            render_pkg["ellipse_area_px"],
            render_pkg["ellipse_area_frac"]
        )
        ...
        frac_threshold = 500 / float(H * W)

        size_pen = size_penalty_power(ellipse_area_frac, visibility_filter, frac_threshold, p=2.0)

        # 权重（可做warmup/余弦调度）
        w_size = 0.1                    # 初值（建议从小到中等，按曲线看再调）
        # 示例：训练前 2k 步从 0 → 0.1 线性升温
        if iteration < 2000:
            w_size = 0.1 * (iteration / 2000.0)

        # 合并总损失
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * dssim + w_size * size_pen

        # 反传前先保留梯度
        size_pen.retain_grad()
        loss.backward()

        if iteration % 100 == 0:
            print(f"Iteration: {iteration}")
            print(f"Loss: {loss.item()}")
            print(f"Ll1: {Ll1.item()}")
            print(f"Dssim: {dssim.item()}")
            print(f"Size penalty: {size_pen.item()}")
            print(f"Gradient w.r.t. size_pen: {size_pen.grad}")
        ...
        with torch.no_grad():
            ...
            if in_window:
                ...
                do_profile_prune = end_of_round and (rounds_seen % max(1, rounds_per_prune) == 0 ) and (not prune_sched.get("mass_stop", False)) 
                ...
                if do_profile_prune and cycle_stats["seen_views"].sum() > 0:
                    ...
                    # 四项分数（映射到 [0,1]，越大越该删）
                    s_w   = robust_norm(mean_w_px,        q_lo=q,   q_hi=1-q,   invert=False)   # 低贡献低分
                    s_t1  = robust_norm(top1_rate,        q_lo=q,   q_hi=1-q,   invert=False)   # 低主导低分
                    s_pxM = robust_norm(px_max,           q_lo=q,   q_hi=1-q,   invert=False)   # 小尺寸低分
                    s_pv  = robust_norm(px_per_used_view, q_lo=3*q, q_hi=1-3*q, invert=True)  # 大块低分

                    # 权重（可调）
                    w_w, w_t1, w_pxM, w_pv = 0.4, 0.1, 0.25, 0.25
                    ...
                    if k_cap > 0 and cand_idx.numel() > 0:
                        # 升序，低分先删
                        order = torch.argsort(scores_sub, descending=False)
                        ...
                        # ===== 重要性质量占比判断：这批 pick 的质量是否 ≥ 1%？ =====
                        MASS_THRESH = 0.1
                        # 用你当前的 prune_score_all 作为“可删性分”，把它当成反向重要性的 proxy。
                        # 如果你希望“重要性=保留度”，可以用 1 - prune_score_all，这里沿用你上面定义的 prune_score_all 即可。
                        pick_mass  = float(prune_score_all[pick].sum().item())
                        total_mass = float(prune_score_all.sum().item()) + 1e-12
                        mass_frac  = pick_mass / total_mass
                        print(f"pick_mass: {pick_mass}, total_mass: {total_mass}, mass_frac: {mass_frac}")

                        if mass_frac >= MASS_THRESH:
                            # 触发：这批次就已经占到了≥1%的“总质量”，按你的要求——停止裁剪，并“再也不进行”
                            prune_sched["mass_stop"] = True

                            # 打印并跳过本轮真正删除（不调用 prune_points）
                            print(
                                f"\n[ITER {iteration}] MASS-STOP TRIGGERED: "
                                f"batch_mass={pick_mass:.6g} ({mass_frac:.2%} of total). "
                                f"Stop all future pruning."
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_size_penalty", type=float, default=0.01,
                        help="Weight for Gaussian size penalty based on covered pixels")
    parser.add_argument("--size_threshold_ratio", type=float, default=5000,
                        help="Pixel threshold ratio for size penalty (image_pixels / ratio)")
    parser.add_argument("--size_penalty_type", type=str, default="quadratic", 
                        choices=["linear", "quadratic", "exponential"],
                        help="Type of size penalty: linear, quadratic, or exponential")
