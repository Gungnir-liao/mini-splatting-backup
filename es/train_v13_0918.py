...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # === 新增：构造前重后轻的轮次权重（几何/多项式/余弦，任选一个；默认几何更好用） ===
    def _frontload_weights(R: int, tail_ratio: float = 0.01, beta: float = 0.10):
        """
        R: 总轮数
        tail_ratio: 最后一轮权重 ≈ 第一轮权重的比例（例如 0.01=1%）
        beta: 均匀底座比例（0~0.3），避免尾部权重过小或为 0
        """
        R = max(1, int(R))
        if R == 1:
            return [1.0]
        lam = float(tail_ratio ** (1.0 / (R - 1)))  # 反推几何 λ
        r = torch.arange(1, R + 1, dtype=torch.float64)
        w = lam ** (r - 1)                          # 几何前重
        w = w / w.sum()
        if beta > 0.0:
            w = (1.0 - beta) * w + beta / R         # 加均匀底座
        w = (w / w.sum()).tolist()
        return w

    weights = _frontload_weights(
        R=effective_rounds,
        tail_ratio=0.01,   # ← 尾/头 ≈ 1%（更保守可 0.03，更激进可 0.005）
        beta=0.10          # ← 10% 均匀底座，保证尾部>0
    )
    prune_sched = {
        "N0": None,
        "removed_so_far": 0,
    ...
    print(f"[PRUNE SCHED] rounds={effective_rounds}, head={weights[0]:.4f}, tail={weights[-1]:.4f}, "
        f"tail/head={(weights[-1]/weights[0]):.3%}, sum={sum(weights):.6f}")
    # ================================================================
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # ==================== 2) 标准梯度致密化（原版逻辑） ====================
            if iteration <= PROFILE_PRUNE_START_ITER and gaussians.get_xyz.shape[0] < 4_000_000:
                gaussians.max_radii2D[visibility_filter] = torch.max(
                    gaussians.max_radii2D[visibility_filter], radii[visibility_filter]
                )
                gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter)
            ...
            if in_window:
                ...
                if do_profile_prune and cycle_stats["seen_views"].sum() > 0:
                    ...
                    # === 新版 per-round 上限（前重后轻 + 动态按缺口比例缩放） ===
                    R = prune_sched["total_rounds"]
                    r = prune_sched["round_idx"]                      # 1-based
                    weights = prune_sched["weights"]
                    T = prune_sched["target"]                         # 例如 0.85

                    # 本轮“计划份额”（占 N0 的比例）
                    plan_frac_this = T * weights[r - 1]

                    # 后续（含本轮）剩余的计划份额之和
                    plan_frac_remaining = T * sum(weights[r - 1:])

                    # 把“实际缺口 / 计划缺口”作为缩放系数，既能吃掉前面没删够的量，又不至于尾部暴冲
                    scale = 0.0 if plan_frac_remaining <= 1e-8 else (remaining_target / plan_frac_remaining)

                    # 动态本轮上限（可再加一个尾部放大上限，避免最后几轮突然很大）
                    round_frac = plan_frac_this * scale
                    round_frac = min(round_frac, prune_sched["max_late_mult"] * plan_frac_this)
                    ...
                    N0 = prune_sched["N0"]
                    k_cap_abs = int(round(round_frac * N0))                 # 本轮最多删这么多（绝对数）
                    k_cap_abs = min(k_cap_abs, int(round(remaining_target * N0)))  # 别超过总体缺口
                    ...
                    # 权重（可调）
                    w_w, w_t1, w_pxM, w_pv = 0.40, 0.10, 0.25, 0.25

                    # 低梯度门控（早期收敛期更稳；到点后可放开）
                    if iteration < (PROFILE_PRUNE_START_ITER + 5000):
                        gate = low_mask.float()
                    else:
                        gate = torch.ones_like(s_w)
                    ...
                    if k_cap > 0 and cand_idx.numel() > 0:
                        ...
                        print(
                            ...
                            f"cap_abs={k_cap_abs} (~{(k_cap_abs / N0):.4%} of N0, remain={remaining_target}), "
