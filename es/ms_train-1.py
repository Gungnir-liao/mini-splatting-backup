...
def progressive_lambda(iteration, t0, t1, lambda_max=1e-4, alpha=3.0):
    """渐进式权重因子: 从 iteration=t0 到 t1 逐渐增加"""
    if iteration < t0:
        return 0.0
    elif iteration > t1:
        return lambda_max
    else:
        progress = (iteration - t0) / (t1 - t0)
        return lambda_max * (progress ** alpha)


def gaussian_size_regularizer(gaussians, render_pkg, iteration,
                              t0=1000, t1=10000, lambda_max=1e-4):
    # scales: [N,3]
    scales = gaussians.get_scaling
    sigma_geo = (scales.prod(dim=1) + 1e-12).pow(1/3)   # [N]
    mask = render_pkg["visibility_filter"].float()      # [N]
    w = mask / (mask.sum() + 1e-6)

    L_size = (w * (1.0 / (sigma_geo + 1e-6))).sum()

    lam = progressive_lambda(iteration, t0, t1, lambda_max)
    return lam * L_size
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.simp_iteration1 < iteration <= args.simp_iteration2:

            loss_reg = gaussian_size_regularizer(gaussians, render_pkg, iteration,
                                                t0=args.simp_iteration1,
                                                t1=args.simp_iteration2,
                                                lambda_max=1e-4)

            loss = loss + loss_reg
            print(f"Loss reg: {loss_reg.item()}") if iteration % 100 == 0 else None
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and iteration % 5000!=0 and gaussians._xyz.shape[0]<args.num_max:  
                    ...
                    if size_threshold is not None:
                        progress = (iteration - 0) / (opt.densify_until_iter - 0)
                        size_threshold = progress * 30


                    gaussians.densify_and_prune_split_merge(opt.densify_grad_threshold, 
