        ...
        with torch.no_grad():
            ...
            if in_window:
                ...
                if do_profile_prune and cycle_stats["seen_views"].sum() > 0:
                    ...
                    eps0 = 0.0  # 你的 used_views/seen_views 是整型计数的 float32，这里用 ==0 即可
                    never_used_mask = (cycle_stats["used_views"] <= eps0)     # 本轮没有覆盖过任何像素
                    never_seen_mask = (cycle_stats["seen_views"] <= eps0)     # 本轮没有被任何帧看到
                    ...
                    th_pv   = torch.quantile(px_per_used_view, 1 - 3*q) # 大块
                    ...
                    cands = (c1 | c2 | c3 | c4)  if iteration < PROFILE_PRUNE_START_ITER + 5000 else (c1 | c2 | c3 | c4)
                    ...
                    k_cap_abs = int(round(p_cap_now * N0))                                 # 本轮最多可删（按 N0 均摊）
                    k_cap_abs = min(k_cap_abs, remaining_needed)                           # 不能超过剩余目标
                    ...
                    if k_cap > 0 and cand_idx.numel() > 0:
                        # ======== 词典序：先 mean_w_px 升序；并列再按 px_per_used_view 降序 ========
                        sub_w  = mean_w_px[cand_idx]
                        sub_pv = px_per_used_view[cand_idx]

                        # rank_w：mean_w_px 从小到大 -> 0..N-1（越小越靠前）
                        w_order = torch.argsort(sub_w, descending=False)
                        rank_w = torch.empty_like(w_order)
                        rank_w[w_order] = torch.arange(w_order.numel(), device=sub_w.device)

                        # rank_pv_desc：px_per_used_view 从大到小 -> 0..N-1（越大越靠前）
                        pv_order_desc = torch.argsort(sub_pv, descending=True)
                        rank_pv_desc = torch.empty_like(pv_order_desc)
                        rank_pv_desc[pv_order_desc] = torch.arange(pv_order_desc.numel(), device=sub_pv.device)

                        # 组合键实现严格词典序
                        Ncand = cand_idx.numel()
                        combined = rank_w * (Ncand + 1) + rank_pv_desc
                        order = torch.argsort(combined, descending=False)
            ...
            if iteration > (PROFILE_PRUNE_START_ITER + PRUNE_WINDOW_ITERS):
                ...
                if iteration % 23000 == 0 or iteration % 26000 == 0:                    
