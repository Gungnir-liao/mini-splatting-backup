        ...
        with torch.no_grad():
            ...
            if in_window:
                ...
                if do_profile_prune and cycle_stats["seen_views"].sum() > 0:
                    ...
                    th_pv   = torch.quantile(px_per_used_view, 0.6) # 大块
                    ...
                    if k_cap > 0:
                        # 简单稳妥：按 mean_w_px 从低到高挑前 k_cap
                        order = torch.argsort(mean_w_px[cand_idx], descending=False)
            ...
            # ==================== 4) 最后做一些小修小补 ====================
            if  PROFILE_PRUNE_START_ITER + PRUNE_WINDOW_ITERS <= iteration < 25_000:
                gaussians.max_radii2D[visibility_filter] = torch.max(
                    gaussians.max_radii2D[visibility_filter], radii[visibility_filter]
                )
                gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter)

                if iteration % (num_train_cams * 2) == 0:
                    size_threshold = 20
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.002,
                                                scene.cameras_extent, size_threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_000, 20_000, 25_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[7_000, 20_000, 25_000])
