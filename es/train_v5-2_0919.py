...
# ★ 1. 直接在此处定义退火策略的全部参数
# -----------------------------------------------------------------
# 从哪一轮开始进行退火裁剪 (通常设置为 densify_until_iter)
ANNEAL_START_ITER = 10000 
# 退火过程持续多少轮
ANNEAL_DURATION = 15000
# 开始时的分位数 (较激进，例如裁剪底部10%的弱点)
START_PRUNE_Q = 0.15
# 结束时的分位数 (较保守，例如只裁剪底部2%的弱点)
END_PRUNE_Q = 0.01
# 开始时单次裁剪的点数比例上限 (例如最多裁剪总数的5%)
START_PRUNE_P_CAP = 0.25
# 结束时单次裁剪的点数比例上限 (例如最多裁剪总数的1%)
END_PRUNE_P_CAP = 0.02
# -----------------------------------------------------------------


def get_annealed_pruning_params(iteration):
    """
    根据当前迭代次数和上面定义的常量，计算退火后的裁剪参数。

    Args:
        iteration: 当前的训练迭代次数。

    Returns:
        A dictionary containing the annealed parameters:
        {'q_w': float, 'q_dom': float, 'p_cap': float}
    """
    start_iter = ANNEAL_START_ITER
    duration = ANNEAL_DURATION
    end_iter = start_iter + duration

    # 1. 计算退火进度 (progress)，范围从 0.0 到 1.0
    if iteration < start_iter:
        progress = 0.0
    elif iteration > end_iter:
        progress = 1.0
    else:
        progress = (iteration - start_iter) / duration

    # 2. 使用线性插值计算当前的参数值
    # current = start * (1 - progress) + end * progress
    current_q = START_PRUNE_Q * (1.0 - progress) + END_PRUNE_Q * progress
    current_p_cap = START_PRUNE_P_CAP * (1.0 - progress) + END_PRUNE_P_CAP * progress

    return {
        'q_w': current_q,
        'q_dom': current_q,  # q_w 和 q_dom 使用相同的退火策略
        'p_cap': current_p_cap
    }
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    mask_big_gs = torch.zeros(gaussians._xyz.shape[0], device='cuda')
    # ==== [MOD-END] ===========================================================
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                cover_px = render_pkg["area_proj"]
                mask_big_gs = torch.logical_or(mask_big_gs, cover_px>(image.shape[1]*image.shape[2]/10000))
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and gaussians._xyz.shape[0]<4_500_000:
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    gaussians.densify_and_prune_split(opt.densify_grad_threshold, 
                                                    0.005, scene.cameras_extent, 
                                                    size_threshold, mask_big_gs)
                    if iteration % 100 == 0:
                        print(f"[ITER {iteration}] mask_big_gs: {mask_big_gs.sum()}")
                    mask_big_gs = torch.zeros(gaussians._xyz.shape[0], device='cuda')
            ...
            if iteration > opt.densify_until_iter:
                ...
                do_round_densify = (
                    end_of_round 
                    and iteration < opt.densify_until_iter+19000
                    and rounds_seen > 0
                ...
                if do_round_densify:
                    ...
                    if getattr(pipe, "collect_stats", False) and cycle_hits.sum() > 0:
                        pruning_params = get_annealed_pruning_params(iteration) # 无需传入 opt
                        q_w = pruning_params['q_w']
                        q_dom = pruning_params['q_dom']
                        p_cap = pruning_params['p_cap']

                        print(f"[ITER {iteration}] Annealed Pruning Params: q={q_w:.4f}, p_cap={p_cap:.4f}")
